    ...
    public IDChartItemDataDTO getChartAggregationResult(IDChartItem chartItem,String dashboardId,String layerId, List<FilterConfig> filters) throws NoSuchFieldException, IllegalAccessException {
        ...
        switch (genericChartItemType) {
            ...
            case FUSION_CHART_VERTICAL_GROUP:
                ...
                break;
            case FUSION_CHART_VERTICAL_GROUP2:
                chartData = getDrillDownChart(chartItem, fields, source.getCollectionName(), filters);
                List<ChartData> chartData1 = (List<ChartData>) chartData;
                Categories categoriesData1 = new Categories();
                DataSet dataSet = new DataSet();
                List<DataValue> dataValues = new ArrayList<>();

                ((List<ChartData>) chartData).stream().forEach(data1->{
                    categoriesData1.getCategory().add(
                            new Category().setLabel(data1.getName())
                    );
                    var seriesDataMap = data1.getSeries();
                    Integer valuefromSeries = 0;
                    for(int i=0;i<seriesDataMap.size();i++){
                        valuefromSeries += (Integer) seriesDataMap.get(i).getValue();
                    }
                    DataValue dataValue = new DataValue();
                    dataValue.setValue(valuefromSeries);
                    dataValues.add(dataValue);
                });
                dataSet.setSeriesname("Total Test case");
                dataSet.setData(dataValues);
                YaxisCombineFusionChartData stackedFusionChartData1 = new YaxisCombineFusionChartData( "#chart_ui_data",List.of(categoriesData1),dataSet);
                JSONArray jsonArray11 = new JSONArray();
                jsonArray11.add(stackedFusionChartData1);
                dataDTO.setData(jsonArray11);
                break;
            case LINESERIES_FUSION_CHART2:
                chartData = getLineChart(chartItem, fields, source.getCollectionName(), filters);
                List<Object[]> list = new ArrayList<>();
                ((List<ChartData>) chartData).stream().forEach(data->{
                    String strdate = data.getName();
                    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("EEE MMM dd HH:mm:ss zzz yyyy", Locale.ENGLISH);
                    ZonedDateTime date = ZonedDateTime.parse(strdate, formatter);
                    DateTimeFormatter outputFormatter = DateTimeFormatter.ofPattern("dd-MMM-yy", Locale.ENGLISH);
                    String formattedDate = date.format(outputFormatter);
                    list.add(new Object[]{formattedDate, data.getValue()});
                });
                ...
                for (Object[] array : list) {
                    ...
                    // If the date is already in the map, add the value to the existing sum
                    if (sums.containsKey(date)) {
                        sums.put(date, sums.get(date) + value);
                    } else {
                        // Otherwise, add the date to the map with the current value
                        sums.put(date, value);
                    }
                }
                List<List<Object>> list2 = new ArrayList<>();
                for (Map.Entry<String, Integer> entry : sums.entrySet()) {
                    list2.add(Arrays.asList(entry.getKey(), entry.getValue()));
                }

                dataDTO.setData(list2);
                break;
            case BAR_CHART:
                    chartData = getBarChart(chartItem, fields, source.getCollectionName(), filters);
                    dataDTO.setData(chartData);
